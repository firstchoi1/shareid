...
export function ShowcaseHome({
  ...
  const load = useCallback(async (nextRegion: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/showcase/accounts?region=${encodeURIComponent(nextRegion)}`, {
        cache: "no-store",
      });
  ...
  const copyAccountPassword = useCallback((accountId: number, password: string) => {
  ...
  const purchaseCta = (
    <div className="mx-auto flex w-full max-w-2xl items-center justify-center gap-1.5 sm:gap-2.5">
      <div className="flex shrink-0 items-center -space-x-2.5 sm:-space-x-3" aria-hidden>
                  ...
                  {rows.map((account) => (
                    ...
                    >
                      ...
                      <div className="flex gap-2 border-t border-slate-200/90 bg-white/90 px-3 py-3 sm:px-3.5">
                        ...
                        <button
                          type="button"
                          onClick={() => copyAccountPassword(account.id, account.password)}
                      ...
                      {copied === `u-${account.id}` || copied === `p-${account.id}` ? (
        ...
        >
          <div className="w-full max-w-xl overflow-hidden rounded-[28px] bg-white shadow-[0_25px_80px_rgba(15,23,42,0.35)]">
            <div className="px-6 py-7 sm:px-8">
              <h3 id="password-quiz-title" className="text-center text-3xl font-extrabold text-red-600">
                安全提问
              </h3>
